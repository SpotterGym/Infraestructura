...
Vagrant.configure("2") do |config|
  ...
  config.vm.network "forwarded_port", guest: 3031, host: 3031, auto_correct: true
  ...
  config.vm.provision :shell, path: "scripts/dockerHub.sh", run: "always"
  # config.vm.provision :shell, path: "../Docker/spotter-deploy.yml", run: "always"
